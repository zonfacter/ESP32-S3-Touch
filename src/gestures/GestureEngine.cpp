...
GestureEvent GestureEngine::process(const TouchPoint pts[MAX_TOUCH_POINTS], uint8_t active){
  ...
  g.timestamp = millis(); 
  g.finger_count = active;

  // IDs der ersten drei aktiven Finger
  int id[3]; int k=0;
  for (int i=0;i<MAX_TOUCH_POINTS && k<3;i++) if (pts[i].active) id[k++]=i;

  // ------------------------------------------------------------
  // 0 FINGER: finalisiere 1-Finger (Tap/Swipe) + ggf. Two-Finger-Tap
  // ------------------------------------------------------------
  if (active == 0){
    if (one_active){
      ...
      if (!one_committed){
        ...
        if (dt <= TAP_MAX_DURATION && adx <= TAP_MAX_MOVEMENT && ady <= TAP_MAX_MOVEMENT){
          if (lastTapTime > 0 && (g.timestamp - lastTapTime) <= DOUBLE_TAP_INTERVAL){
            ...
            g.x = one_lx; g.y = one_ly; 
            lastTapTime = 0;
          } else {
            g.type = GestureType::Tap; 
            g.x = one_lx; g.y = one_ly; 
            lastTapTime = g.timestamp; 
            lastTapX = one_lx; lastTapY = one_ly;
          }
        } else {
          // Swipe final (nur klare Achse)
          if (fdist(one_lx,one_ly,one_sx,one_sy) >= SWIPE_MIN_DISTANCE){
            if (adx > ady * SWIPE_AXIS_RATIO) 
              g.type = (dx>0)?GestureType::SwipeRight:GestureType::SwipeLeft;
            else if (ady > adx * SWIPE_AXIS_RATIO) 
              g.type = (dy>0)?GestureType::SwipeDown:GestureType::SwipeUp;

            if (g.type != GestureType::None){
              g.x = one_lx; g.y = one_ly; 
              g.value = fdist(one_lx,one_ly,one_sx,one_sy);
            }
          }
        }
      }
    ...
    // Two-Finger-Tap nur wenn in 2-Finger-Phase nichts committed wurde
    if (two_active && !two_committed){
      if (g.type == GestureType::None){
        g.type = GestureType::TwoFingerTap; 
        g.x = two_cx; g.y = two_cy;
      }
    }
  ...
  // ------------------------------------------------------------
  // 1 FINGER: Long-Press live (einmalig), Tap/Swipe erst bei Release
  // ------------------------------------------------------------
  if (active == 1){
    auto &p = pts[id[0]];
    ...
    return g; // Tap/Swipe kommen beim Loslassen (active==0)
  }

  // ------------------------------------------------------------
  // 2+ FINGER: Pinch/Rotate (einmalig), 1-Finger wird verworfen
  // ------------------------------------------------------------
  if (active >= 2){
    ...
    if (!two_committed){
      ...
      if (fabsf(dd) >= PINCH_THRESHOLD){
        g.type = (dd<0)?GestureType::PinchIn:GestureType::PinchOut; 
        g.x=cx; g.y=cy; g.value=fabsf(dd);
        two_committed = true; 
        return g;
      }
      if (fabsf(da) >= (ROTATE_THRESHOLD * (M_PI/180.f))){
        g.type = (da>0)?GestureType::RotateCCW:GestureType::RotateCW; 
        g.x=cx; g.y=cy; g.value=fabsf(da);
        two_committed = true; 
        return g;
      }
    }

    // Während 2-Finger aktiv -> 1-Finger ignorieren
    one_active = false; one_committed = false;
    return g;
  }
